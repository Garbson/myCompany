      ...
      >
        <div class="glass-strong rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
          <header class="flex items-center justify-between px-5 py-3 border-b border-white/5">
            <div>
              <h2 class="text-sm font-semibold text-white">Fundo do site</h2>
              <p class="text-[11px] text-gray-500">Escolha um vídeo para rodar atrás das páginas</p>
            </div>
            <button
              @click="$emit('close')"
              class="text-gray-400 hover:text-white p-1 -m-1 transition-colors"
              aria-label="Fechar"
            >
          ...
          <div class="px-5 py-3 border-b border-white/5 flex items-center gap-2 text-[11px] text-gray-400">
            <span
          ...
          <div class="flex-1 overflow-y-auto p-4 grid grid-cols-2 md:grid-cols-3 gap-3 scrollbar-slim">
            ...
            >
              ...
              <div class="absolute inset-x-0 bottom-0 px-2 py-1.5 bg-gradient-to-t from-black/80 to-transparent">
                <p class="text-xs font-medium text-white text-left">{{ bg.label }}</p>
              </div>
              <div
