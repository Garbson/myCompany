<template>
  <aside
    class="hidden md:flex shrink-0 w-60 h-full paper-strong flex-col overflow-hidden rounded-2xl"
  >
    <!-- Brand -->
    <div class="px-5 pt-5 pb-4 flex items-center gap-3 border-b border-[var(--paper-border)]">
    ...
    <nav class="flex-1 p-3 space-y-0.5 overflow-y-auto overflow-x-hidden scrollbar-slim">
      <template v-for="item in menu" :key="item.path || item.action">
        ...
        <button
          ...
          class="group w-full flex items-center gap-3 px-3 py-2 rounded-lg text-[13px] text-ink-100 hover:bg-[var(--paper-surface-3)] hover:text-ink-400 transition-colors whitespace-nowrap"
        >
          ...
          <span v-if="item.action === 'bg' && currentBgId !== 'none'" class="ml-auto w-1.5 h-1.5 rounded-full bg-olive-500"></span>
        </button>
        <!-- Item de rota -->
        <router-link
          v-else
          :to="item.path"
          class="group flex items-center gap-3 px-3 py-2 rounded-lg text-[13px] text-ink-100 hover:bg-[var(--paper-surface-3)] hover:text-ink-400 transition-colors whitespace-nowrap"
          :class="{
            'bg-[var(--paper-surface-3)] text-ink-400 font-semibold':
              isActive(item.path),
          }"
        >
          <span
    ...
    <!-- User -->
    <div class="p-3 border-t border-[var(--paper-border)] relative">
      <button
        @click="showMenu = !showMenu"
        class="w-full flex items-center gap-2.5 p-2 rounded-lg hover:bg-[var(--paper-surface-3)] transition-colors"
      >
        <div
      ...
      >
        <div
          v-if="showMenu"
          class="absolute bottom-full left-3 right-3 mb-2 paper-strong rounded-xl overflow-hidden"
        >
          <button
            @click="confirmLogout"
      ...
      <p class="text-[9px] text-ink-50 text-center mt-2 font-mono">{{ versionText }}</p>
    </div>
