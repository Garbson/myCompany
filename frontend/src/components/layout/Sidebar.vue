<template>
  ...
  <aside
    ...
    style="top: env(safe-area-inset-top, 0px); padding-bottom: env(safe-area-inset-bottom, 0px)"
    @mouseenter="onHover(true)"
    @mouseleave="onHover(false)"
  >
    <!-- Logo -->
    <div class="px-2.5 py-3 border-b border-gray-800 flex items-center gap-2 min-h-[56px]">
      <img src="/logo.svg" alt="myCompany" class="w-8 h-8 shrink-0" />
      <div class="overflow-hidden whitespace-nowrap transition-all duration-300 ease-in-out" :class="sidebarCompact ? 'w-0 opacity-0' : 'w-auto opacity-100'">
    ...
    <nav class="flex-1 p-2 space-y-0.5 overflow-y-auto overflow-x-hidden">
      ...
      >
        <span class="text-base shrink-0 flex items-center justify-center w-4">{{ item.icon }}</span>
        <span class="text-[13px] transition-all duration-300 ease-in-out" :class="sidebarCompact ? 'opacity-0 -translate-x-2' : 'opacity-100 translate-x-0'">{{ item.label }}</span>
      </router-link>
    </nav>
  ...
  <button
    ...
    style="top: calc(env(safe-area-inset-top, 0px) + 0.75rem); left: calc(env(safe-area-inset-left, 0px) + 0.75rem)"
  >
