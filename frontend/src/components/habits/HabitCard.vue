  ...
  >
    ...
    <button
      ...
      :class="habit.completed_today
        ? 'bg-olive-500 border-green-500 shadow-lg shadow-green-500/30'
    ...
    >
      <svg v-if="habit.completed_today" class="w-6 h-6 text-ink-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3">
        <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
      </svg>
    </button>

    <div v-else class="w-12 h-12 rounded-full shrink-0 flex items-center justify-center bg-[var(--paper-surface-2)]">
    ...
    <div class="flex-1 min-w-0">
      <div class="flex items-start justify-between gap-3">
        <div class="min-w-0">
          <p class="text-sm font-semibold text-ink-400" :class="{ 'line-through': habit.completed_today }">
            {{ habit.name }}
          </p>
      ...
      <div class="flex flex-wrap gap-1.5 mt-2">
        <span v-if="cueText" class="text-[10px] px-2 py-0.5 rounded bg-terra-500/15 text-terra-500">
          📌 {{ cueText }}
        </span>
        ...
        <span v-if="habit.required_30d > 0" class="text-[10px] px-2 py-0.5 rounded bg-[var(--paper-surface-2)] text-ink-100">
    ...
    <button
      ...
      class="shrink-0 self-start opacity-0 group-hover:opacity-100 transition-opacity text-ink-50 hover:text-ink-400 p-1 -m-1"
      title="Editar"
    >
