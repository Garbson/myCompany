<template>
  <div>
    <div class="flex items-center justify-between mb-6 md-sticky-title">
      <h1 class="text-xl md:text-2xl font-semibold text-ink-400 tracking-tight">Freelas</h1>
      <button @click="openCreateProject" class="px-4 py-2 bg-terra-500 text-ink-400 text-sm font-medium rounded-lg hover:bg-terra-600">
        + Novo projeto
      </button>
    </div>
    ...
    <div class="space-y-3">
      <div v-for="project in freelasProjects" :key="project.id" class="glass rounded-xl glow-hover overflow-hidden">
        <div class="p-4 cursor-pointer hover:bg-[var(--paper-surface-2)] transition-colors" @click="toggleProject(project)">
          <div class="flex items-center justify-between gap-3">
            <div class="flex items-center gap-3 min-w-0 flex-1">
              ...
              <div class="min-w-0">
                ...
                <p class="text-xs text-ink-50">{{ project.lead_name || 'Sem lead vinculado' }}</p>
              </div>
            </div>
            <div class="hidden md:flex items-center gap-3 shrink-0">
              <div class="text-right">
                <p class="text-sm font-semibold text-ink-300">
                  ...
                  <span v-if="project.monthly_fee > 0" class="text-xs text-terra-500">+ {{ formatCurrency(project.monthly_fee) }}/mês</span>
                </p>
                <div class="flex items-center gap-1.5 text-[10px]">
                  <span class="px-1.5 py-0.5 rounded text-ink-400 font-medium" :class="paymentTypeBadge(project.payment_type)">{{ paymentTypeLabel(project.payment_type) }}</span>
                  <span class="text-olive-500">Pago: {{ formatCurrency(project.total_paid || 0) }}</span>
                </div>
              </div>
            ...
            <svg class="w-4 h-4 text-ink-50 shrink-0 transition-transform" :class="{ 'rotate-180': expanded === project.id }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
            </svg>
          </div>
          <div class="md:hidden flex items-center gap-2 mt-2 text-xs">
            <span class="px-1.5 py-0.5 rounded text-ink-400 font-medium text-[10px]" :class="paymentTypeBadge(project.payment_type)">{{ paymentTypeLabel(project.payment_type) }}</span>
            <span class="text-ink-100">{{ formatCurrency(project.total_value) }}</span>
            <span v-if="project.monthly_fee > 0" class="text-terra-500">+{{ formatCurrency(project.monthly_fee) }}/mês</span>
        ...
        <div v-if="expanded === project.id" class="border-t border-[var(--paper-border)] p-4 bg-[var(--paper-bg)]/40">
          <div class="flex items-center justify-between mb-3">
            <h4 class="text-xs font-semibold text-ink-100">{{ isMonthlyProject(project) ? 'Mensalidades' : 'Parcelas' }}</h4>
            <button @click="openAddPayment(project)" class="text-xs px-3 py-1.5 bg-terra-500 text-ink-400 rounded-lg hover:bg-terra-600">
          ...
          <div class="hidden md:block overflow-x-auto">
            <table class="w-full text-sm">
              <thead>
                <tr class="text-left text-ink-50 border-b border-gray-800">
              ...
              <tbody>
                <tr v-for="p in payments[project.id]" :key="p.id" class="border-b border-gray-800/50" :class="{ 'opacity-50': p.status === 'pago' }">
                  <td class="py-2 text-ink-200">{{ p.installment_number }}</td>
                  <td class="py-2 text-ink-200">{{ formatDate(p.due_date) }}</td>
                  <td class="py-2 text-ink-300 font-medium">{{ formatCurrency(p.amount) }}</td>
                  ...
                  <td class="py-2 text-ink-50 max-w-[120px] truncate">{{ p.notes || '-' }}</td>
                  <td class="py-2 text-right">
                    <button v-if="p.status !== 'pago'" @click="markAsPaid(p)" class="text-xs px-2 py-1 bg-olive-500/20 text-olive-500 rounded hover:bg-green-500/30">Marcar pago</button>
                    <span v-else class="text-xs text-green-500">Pago {{ formatDate(p.paid_at) }}</span>
                  </td>
                </tr>
                <tr v-if="!payments[project.id]?.length"><td colspan="6" class="py-4 text-center text-ink-50">Nenhum registro</td></tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="md:hidden space-y-2">
            <div v-for="p in payments[project.id]" :key="p.id" class="glass-light rounded-lg glow-hover p-3 flex items-center justify-between" :class="{ 'opacity-50': p.status === 'pago' }">
              <div>
                <p class="text-sm text-ink-300">#{{ p.installment_number }} — {{ formatCurrency(p.amount) }}</p>
                <p class="text-xs text-ink-50">Venc: {{ formatDate(p.due_date) }}</p>
                <span class="text-[10px] px-1.5 py-0.5 rounded-full font-medium" :class="paymentBadge(p.status)">{{ paymentLabel(p.status) }}</span>
              </div>
              <button v-if="p.status !== 'pago'" @click="markAsPaid(p)" class="text-xs px-2 py-1 bg-olive-500/20 text-olive-500 rounded">Pagar</button>
              <span v-else class="text-xs text-green-500">Pago</span>
            </div>
            <p v-if="!payments[project.id]?.length" class="text-center text-ink-50 text-sm py-4">Nenhum registro</p>
          </div>

          <div v-if="payments[project.id]?.length" class="mt-3 pt-3 border-t border-gray-800 flex justify-end gap-4 text-xs">
            <span class="text-ink-50">Pendente: <strong class="text-[#C89A3F]">{{ formatCurrency(sumPending(project.id)) }}</strong></span>
            <span class="text-ink-50">Pago: <strong class="text-olive-500">{{ formatCurrency(sumPaid(project.id)) }}</strong></span>
            <span class="text-ink-50">Faltam: <strong class="text-terra-600">{{ formatCurrency(pendingAmount(project)) }}</strong></span>
          </div>
        </div>
      </div>

      <p v-if="freelasProjects.length === 0 && !projectStore.loading" class="text-center text-ink-50 py-12 text-sm">Nenhum freela cadastrado</p>
    </div>
    ...
    <Modal :show="showProjectModal" :title="editingProject ? 'Editar projeto' : 'Novo projeto'" @close="closeProjectModal">
      <form @submit.prevent="saveProject" class="space-y-4">
        ...
        <div>
          ...
          <textarea v-model="projectForm.description" rows="2" class="w-full px-3 py-2 bg-[var(--paper-surface)] border border-[var(--paper-border)] rounded-lg text-ink-400 focus:outline-none focus:border-indigo_ink-500"></textarea>
        </div>
        <div>
        ...
        <div v-if="hasSetup" class="bg-terra-500/10 border border-terra-500/20 p-3 rounded-lg">
          <label class="block text-sm font-medium text-ink-200 mb-1">Valor do projeto</label>
        ...
        <div v-if="hasInstallments" class="bg-orange-500/10 border border-orange-500/20 p-3 rounded-lg space-y-3">
          <div v-if="!hasSetup">
            <label class="block text-sm font-medium text-ink-200 mb-1">Valor total do projeto</label>
          ...
          <div>
            <label class="block text-sm font-medium text-ink-200 mb-1">Entrada</label>
            <input v-model.number="projectForm.entry_value" type="number" step="0.01" class="w-full px-3 py-2 bg-[var(--paper-surface)] border border-[var(--paper-border)] rounded-lg text-ink-400 focus:outline-none focus:border-indigo_ink-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-ink-200 mb-1">Data da entrada</label>
            <input v-model="projectForm.entry_date" type="date" class="w-full px-3 py-2 bg-[var(--paper-surface)] border border-[var(--paper-border)] rounded-lg text-ink-400 focus:outline-none focus:border-indigo_ink-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-ink-200 mb-1">Parcelar o restante em</label>
            <select v-model.number="projectForm.installments" class="w-full px-3 py-2 bg-[var(--paper-surface)] border border-[var(--paper-border)] rounded-lg text-ink-400 focus:outline-none focus:border-indigo_ink-500">
            ...
            <p v-if="projectTotal > 0 && projectForm.entry_value >= 0 && projectForm.installments > 0" class="text-[10px] text-terra-500 mt-1">
        ...
        <div v-if="hasSingle" class="bg-olive-500/10 border border-green-500/20 p-3 rounded-lg">
          <label class="block text-sm font-medium text-ink-200 mb-1">Valor total</label>
          <input v-model.number="projectForm.total_value" type="number" step="0.01" class="w-full px-3 py-2 bg-[var(--paper-surface)] border border-[var(--paper-border)] rounded-lg text-ink-400 focus:outline-none focus:border-indigo_ink-500" />
        </div>
        <div v-if="hasMonthly" class="bg-purple-500/10 border border-purple-500/20 p-3 rounded-lg space-y-3">
          ...
          <div>
            <label class="block text-xs font-medium text-ink-100 mb-1">Ciclo</label>
            <select v-model="projectForm.monthly_cycle" class="w-full px-3 py-2 bg-[var(--paper-surface)] border border-[var(--paper-border)] rounded-lg text-ink-400 focus:outline-none focus:border-indigo_ink-500">
          ...
          <div v-if="projectForm.monthly_cycle === 'anual'">
            <label class="block text-xs font-medium text-ink-100 mb-1">Parcelar anuidade em</label>
            <select v-model.number="projectForm.annual_installments" class="w-full px-3 py-2 bg-[var(--paper-surface)] border border-[var(--paper-border)] rounded-lg text-ink-400 focus:outline-none focus:border-indigo_ink-500">
            ...
            <p v-if="projectForm.monthly_fee > 0 && projectForm.annual_installments > 1" class="text-[10px] text-terra-500 mt-1">
        ...
        <div>
          ...
          <select v-model="projectForm.status" class="w-full px-3 py-2 bg-[var(--paper-surface)] border border-[var(--paper-border)] rounded-lg text-ink-400 focus:outline-none focus:border-indigo_ink-500">
        ...
        <div class="flex gap-3 pt-2">
          <button v-if="editingProject" type="button" @click="deleteProject" class="px-4 py-2 text-sm text-terra-600 hover:bg-terra-500/10 rounded-lg">Excluir</button>
          <div class="flex-1"></div>
          <button type="button" @click="closeProjectModal" class="px-4 py-2 text-sm text-ink-100 hover:bg-[var(--paper-surface-2)] rounded-lg">Cancelar</button>
    ...
    <Modal :show="showPaymentModal" :title="activeProjectMonthly ? 'Nova mensalidade' : 'Nova parcela'" @close="showPaymentModal = false">
      <form @submit.prevent="savePayment" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-ink-200 mb-1">Valor *</label>
            <input v-model="paymentForm.amount" type="number" step="0.01" required class="w-full px-3 py-2 bg-[var(--paper-surface)] border border-[var(--paper-border)] rounded-lg text-ink-400 focus:outline-none focus:border-indigo_ink-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-ink-200 mb-1">Nº {{ activeProjectMonthly ? 'mês' : 'parcela' }}</label>
            <input v-model="paymentForm.installment_number" type="number" class="w-full px-3 py-2 bg-[var(--paper-surface)] border border-[var(--paper-border)] rounded-lg text-ink-400 focus:outline-none focus:border-indigo_ink-500" />
          </div>
        </div>
        <div>
          <label class="block text-sm font-medium text-ink-200 mb-1">Vencimento *</label>
          <input v-model="paymentForm.due_date" type="date" required class="w-full px-3 py-2 bg-[var(--paper-surface)] border border-[var(--paper-border)] rounded-lg text-ink-400 focus:outline-none focus:border-indigo_ink-500" />
        </div>
        <div>
          <label class="block text-sm font-medium text-ink-200 mb-1">Notas</label>
          <input v-model="paymentForm.notes" type="text" class="w-full px-3 py-2 bg-[var(--paper-surface)] border border-[var(--paper-border)] rounded-lg text-ink-400 focus:outline-none focus:border-indigo_ink-500" placeholder="Ex: Pix, boleto..." />
        </div>
        <div class="flex gap-3 pt-2">
          <div class="flex-1"></div>
          <button type="button" @click="showPaymentModal = false" class="px-4 py-2 text-sm text-ink-100 hover:bg-[var(--paper-surface-2)] rounded-lg">Cancelar</button>
          <button type="submit" class="px-4 py-2 text-sm bg-terra-500 text-ink-400 rounded-lg hover:bg-terra-600">Adicionar</button>
        </div>
      </form>
    </Modal>
  </div>
...
function statusBadge(s) { return { ativo: 'bg-blue-500/20 text-terra-600', concluido: 'bg-olive-500/20 text-olive-500', cancelado: 'bg-terra-500/20 text-terra-600' }[s] || '' }
...
function paymentBadge(s) { return { pendente: 'bg-[#C89A3F]/20 text-[#C89A3F]', pago: 'bg-olive-500/20 text-olive-500', atrasado: 'bg-terra-500/20 text-terra-600' }[s] || '' }
